const KEY='cccs_sttimothy_attendance_v11';
let db=JSON.parse(localStorage.getItem(KEY)||'{"students":[],"attendance":[],"notices":[],"settings":{"school":"Cordova Catholic Cooperative School","late":"07:30"}}');
...
notices.innerHTML=db.notices.slice(-6).reverse().map(n=>`<div class='notice'><span class='ok'>✓ SMS simulated</span><br>${esc(n.message)}<br><small>${fmt(n.at)}</small></div>`).join('')||'<p class=muted>No notifications yet.</p>';renderStudents();renderAttendance();renderBoard();renderReports()}
